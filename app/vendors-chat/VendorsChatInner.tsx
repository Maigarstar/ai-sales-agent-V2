...
const PREVIEW_ASSISTANT_REPLIES = 2;
...
export default function VendorsChatInner() {
  ...
  }, [messages, loading, view, showGate]);
  ...
  useEffect(() => {
    const count = messages.reduce((acc, m) => (m.role === "assistant" ? acc + 1 : acc), 0);
    setAssistantReplyCount(count);

    if (!isSignedIn && view === "chat" && count >= PREVIEW_ASSISTANT_REPLIES) {
      setShowGate(true);
    } else {
      setShowGate(false);
    }
  }, [messages, isSignedIn, view]);
    ...
    try {
      ...
      if (data.ok && data.conversationId) {
        ...
        const firstAssistant: ChatMessage = {
          role: "assistant",
          content:
            "Hello, I am Aura. I help couples build a confident plan, a refined shortlist, and direct connections to trusted vendors. I also help venues and wedding businesses grow through elevated visibility, sharper positioning, and better enquiries. To begin, tell me which one you are, and what you are working on today.",
        };
          ...
          {savedUser?.email ? (
            <div className="mb-3 p-3 rounded-xl bg-gray-50 border border-gray-200">
              ...
              <div className="mt-2 text-[11px] text-gray-500">
                Saved on this device. Multi device sync comes next.
              </div>
            </div>
          ) : (
            <button
          ...
          <div className={`flex-1 flex flex-col justify-center items-center ${isEmbed ? "py-0" : "py-12 sm:px-6 lg:px-8"}`}>
            <div className="sm:mx-auto sm:w-full sm:max-w-md text-center px-4 mb-8">
              ...
              <p className="text-gray-500 mt-4 text-[16px]">
                The concierge connects couples to venues and vendors, and helps wedding businesses grow through smarter enquiries.
              </p>
            </div>
            ...
            <div className="absolute left-0 top-1/2 -translate-y-1/2 flex items-center gap-2">
              {savedUser?.email ? (
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-gray-50 border border-gray-200">
                  <div className="w-7 h-7 rounded-full bg-[#1F4D3E] text-white flex items-center justify-center text-xs font-semibold">
                    {(savedUser.name || "U").slice(0, 1).toUpperCase()}
                  </div>
                  <div className="leading-tight">
                    <div className="text-[12px] font-semibold text-gray-900 truncate max-w-[180px]">
                      {savedUser.name || "Account"}
                    </div>
                    <div className="text-[11px] text-gray-500 truncate max-w-[180px]">{savedUser.email}</div>
                  </div>
                </div>
              ) : (
              ...
              {showGate && (
                <div className="w-full max-w-3xl mx-auto">
                  <div className="mt-2 bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
                    <div className="text-sm font-semibold text-gray-900">Create your account to continue</div>
                    <div className="mt-1 text-sm text-gray-600">
                      Save your chat history, planning tools, and shortlist.
                    </div>
                    ...
                    <div className="mt-4 flex flex-col sm:flex-row gap-2">
                      <button
                        type="button"
                        onClick={() => setIsAuthModalOpen(true)}
                        className="px-4 py-2.5 rounded-xl bg-[#1F4D3E] text-white text-sm font-medium hover:bg-[#163C30]"
                      >
                        Create account
                      </button>
                      ...
                      <a
                        href="/signup"
                        className="px-4 py-2.5 rounded-xl border border-gray-200 bg-white text-sm font-medium text-gray-800 hover:bg-gray-50 text-center"
                      >
                        Open signup page
                      </a>
                    </div>
                    ...
                    <div className="mt-3 text-[12px] text-gray-500">
                      After email verification, you will be taken to onboarding.
                    </div>
                  </div>
                </div>
              )}
          ...
          <div className="w-full max-w-3xl mx-auto">
            ...
            <div className="relative bg-white border border-gray-300 rounded-[26px] shadow-sm focus-within:ring-2 focus-within:ring-[#1F4D3E] focus-within:border-transparent transition-all">
              ...
              <div className="absolute right-2 bottom-1.5 flex items-center gap-2">
                ...
                <button
                  onClick={() => handleSend()}
                  disabled={loading || !input.trim() || (!isSignedIn && showGate)}
            ...
            <div className="mt-3 text-xs text-gray-500 flex items-center justify-between">
              ...
              {savedUser?.email ? (
                ...
                <a href="/signup" className="text-[#1F4D3E] hover:underline">
                  Create account
                </a>
              )}
            </div>
