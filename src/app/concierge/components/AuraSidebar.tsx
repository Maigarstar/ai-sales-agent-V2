...
import BusinessLeadInbox from "./BusinessLeadInbox";
...
interface AuraSidebarProps {
  ...
  shortlist?: any[]; // Added for live collection
  onRemoveFromShortlist?: (id: string) => void; // Added for live collection
}
...
export default function AuraSidebar({
  ...
  return (
    <aside
      className={`fixed left-0 top-0 h-screen border-r transition-all duration-500 ease-in-out flex flex-col z-[110] ${
        sidebarBg
    ...
    >
      ...
      <div className="flex h-24 items-center justify-between px-8 shrink-0">
        <div className={`luxury-serif text-[12px] tracking-[0.3em] uppercase ${textColor}`}>
          {userType === 'business' ? "Partner Portal" : "My Collection"}
        </div>
        <button
          onClick={onToggle}
          className={`p-2 rounded-full hover:bg-current/5 transition-colors ${textColor}`}
        >
          <ChevronLeft size={20} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-8 custom-scrollbar">
        {userType === 'business' ? (
          <div className="space-y-8">
            ...
            <nav className="space-y-2 pt-4 border-t border-current/10">
          ...
          /* CLIENT VIEW: PERSONAL COLLECTION */
          <div className="space-y-10">
            <div>
              ...
              {/* LIVE SHORTLIST MAPPING */}
              <div className="space-y-4">
                {shortlist.length === 0 ? (
                  <p className="text-[10px] uppercase tracking-widest opacity-20 text-center py-4">Collection Empty</p>
                ) : (
                  shortlist.map((item) => (
                    <div 
                      key={item.id} 
                      className={`group p-3 rounded-2xl border transition-all relative ${
                        isLightMode ? "bg-black/5 border-transparent" : "bg-white/5 border-transparent"
                      }`}
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="flex items-center gap-1 opacity-40 text-[9px] uppercase tracking-tighter mt-1">
                            <MapPin size={8} />
      ...
      <div className={`p-8 border-t ${isLightMode ? "border-black/5" : "border-white/5"}`}>
        {shortlist.length > 0 && (
          <button className="w-full mb-6 py-3 rounded-xl bg-[#C5A059] text-white text-[10px] font-black uppercase tracking-[0.2em] shadow-lg hover:brightness-110 transition-all">
