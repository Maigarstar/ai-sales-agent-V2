...
/* =========================================================
   ...
   ========================================================= */
function makeUuid() {
  // Browser supported in modern Chrome
  if (typeof crypto !== "undefined" && "randomUUID" in crypto) {
    return crypto.randomUUID();
  }

  // Fallback UUID v4, good enough for local and server
  const s: string[] = [];
...
export default function VendorsChatInner() {
  ...
  async function startConversationIfNeeded(): Promise<string> {
    ...
    // Always create a UUID first so local state and server inserts never see local_xxx junk
    ...
    try {
      ...
      // If backend returns a different id, adopt it
      ...
      // Offline is fine, we keep the provisional UUID
      return provisionalId;
    }
  }
