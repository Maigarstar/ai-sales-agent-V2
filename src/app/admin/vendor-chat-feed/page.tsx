...
function WorkspaceTabs(props: { active: "leads" | "conversations" | "live" }) {
  ...
  return (
    <div style={{ marginBottom: 18 }}>
      ...
      >
        <div style={{ fontSize: 12, color: "#777" }}>Admin workspace</div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <Link
            ...
            style={props.active === "conversations" ? activeStyle : inactiveStyle}
          >
...
export default function LiveChatFeedPage() {
  ...
  useEffect(() => {
    ...
    async function loadInitial() {
      // FIX: Guard again inside the async function just to be safe for TS
      if (!sb) return; 
  ...
  async function handleDeleteConversation(id: string) {
    ...
    try {
      ...
      setErrorMessage(`Could not delete this conversation: ${error?.message ?? "Unknown error"}`);
    } finally {
      setDeletingId(null);
    }
  }
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 1000, margin: "0 auto" }}>
        ...
        {errorMessage && (
          <div style={{ marginBottom: 12, fontSize: 13, color: "#aa1111" }}>
            {errorMessage}
          </div>
        )}
        ...
        >
          ...
          {loading && filteredConversations.length === 0 && (
            <div style={{ padding: 18, fontSize: 13, color: "#666" }}>
          ...
          {filteredConversations.map((row) => (
            ...
            >
              <div style={{ color: "#777" }}>{formatDateLabel(row.created_at)}</div>
                ...
                <div style={{ marginBottom: 4, fontSize: 12, color: "#555" }}>
                  <span
                    style={{
                      ...
                      borderRadius: 999,
                      backgroundColor:
                        (row.status || "").toLowerCase() === "done" ? "#e4f4ea" : "#ffece3",
                      color:
                        (row.status || "").toLowerCase() === "done" ? "#1d6b3b" : "#cc692b",
                      fontSize: 11,
                    }}
                  >
              ...
              <div style={{ textAlign: "right", color: "#777", fontSize: 12 }}>
