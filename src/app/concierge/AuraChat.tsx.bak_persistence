  ...
  const [messages, setMessages] = useState<any[]>([
    {
      role: "assistant",
      content:
        "Good evening. I am Aura. How may I assist in curating your vision today?",
    },
  ]);
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          content:
            data.reply ||
            "I have curated some refined options for you below.",
        },
      ]);
        ...
        <div className="max-w-3xl mx-auto space-y-10 pb-20">
          {messages.map((m, i) => (
            ...
            >
              ...
              <div
                className={`max-w-[85%] text-base font-light leading-relaxed ${
                  m.role === "user" ? "text-right" : "text-zinc-300"
                }`}
              >
                {m.content}
              </div>
          ...
          {loading && (
            <div className="flex justify-center py-4">
              <p className="text-[10px] uppercase tracking-[0.4em] text-[#C5A059] animate-pulse">
                Aura is sourcing…
              </p>
            </div>
          )}
      ...
      <footer className="p-8 bg-gradient-to-t from-[#0E100F] via-[#0E100F] to-transparent">
        ...
        >
          ...
          <button
            type="submit"
            disabled={loading}
            className="absolute right-3 top-1/2 -translate-y-1/2 p-3 rounded-full bg-[#C5A059] text-black hover:brightness-110 transition"
          >
            <ArrowUp size={16} />
          </button>
        </form>
