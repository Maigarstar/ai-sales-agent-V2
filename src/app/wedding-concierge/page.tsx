...
// Toggle onboarding during development
...
export default function AuraOnboarding() {
  ...
  // Dev bypass: skip onboarding entirely
  useEffect(() => {
    if (DISABLE_ONBOARDING) {
      router.replace("/wedding-concierge/chat");
    }
  }, [router]);
  ...
  // Avoid flash during redirect
  if (DISABLE_ONBOARDING) {
    return (
      <div className="min-h-screen bg-[#0E100F] text-[#F5F5F7] grid place-items-center">
        Entering concierge…
      </div>
    );
  }
