...
const STORAGE_KEY_SESSIONS = "fsw_chat_sessions_v2";
const STORAGE_KEY_CHAT_ACTIVE = "fsw_chat_active_v2";
const STORAGE_KEY_LEAD = "fsw_lead_v2";
const STORAGE_KEY_CONV_PREFIX = "fsw_chat_conv_v2_";
        ...
        if (isBullet || isNumber) {
          return (
            <div key={i} className="flex items-start">
              <span className={`mr-2 mt-1 flex-shrink-0 ${isBullet ? "w-1.5 h-1.5 bg-gray-400 rounded-full mt-2" : "font-semibold text-[#1F4D3E] text-xs min-w-[18px]"}`}>
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[520px] overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="text-[11px] text-gray-500">
            See privacy policy at 5starweddingdirectory.com/privacy
          </div>
        </div>
...
export default function VendorsChatInner() {
  ...
  useEffect(() => {
    ...
    // Create a fresh local chat state immediately
    const initialMsgs: ChatMessage[] = [{ role: "assistant", content: OPENING_MESSAGE }];
  ...
  useEffect(() => {
    ...
    setSessions(next);
  }, [conversationId, messages, intent, lead]);
  ...
  async function startConversationIfNeeded(): Promise<string | null> {
    ...
    // If backend is unavailable, keep local chat usable
    const fallbackId = `local_${Math.random().toString(36).slice(2)}_${Date.now()}`;
  ...
  async function handleSend(manual?: string) {
    ...
    // If no server conversationId, still show local assistant response placeholder
    if (!id) {
      setLoading(false);
      setMessages((prev) =>
        clampMessages([
          ...prev,
          { role: "assistant", content: "I’m here. Please try again in a moment." },
        ], 80)
      );
      return;
    }
  ...
  function captureLead(name: string, email: string) {
    ...
    // Give immediate clarity in the chat
    setMessages((prev) =>
      clampMessages(
        [
          ...prev,
          {
            role: "assistant",
            content: `Perfect, saved as ${email}. Keep going, what would you like next?`,
          },
        ],
        80
      )
  ...
  const quickPrompts = useMemo(() => {
    ...
    // unknown, show mixed couple first
  ...
  const SidebarContent = (
    <div className="h-full flex flex-col">
          ...
          <div className="leading-tight">
            ...
            {lead?.email ? (
              ...
              <div className="text-[11px] text-gray-400 mt-1">
                Save appears after a few replies
              </div>
            )}
          </div>
      ...
      <div className="flex-1 overflow-y-auto bg-white">
        <div className="p-3">
          ...
          <div className="mt-4 text-[11px] text-gray-500 leading-relaxed">
            Chats store in this browser. Account sync arrives when the platform registration is live.
          </div>
        </div>
      </div>
      ...
      <div className="p-3 border-t border-gray-100 bg-white flex justify-between items-center">
        ...
        <a
          href="https://5starweddingdirectory.com"
          target="_blank"
