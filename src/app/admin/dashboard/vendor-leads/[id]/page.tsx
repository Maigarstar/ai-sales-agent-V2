...
import {
  Mail, Phone, MapPin, Send, ArrowLeft, Sparkles, ShieldCheck, UserCheck, LogOut
} from "lucide-react";

/**
 * 1. Type-Safe Dynamic Import
 * Casting as any prevents the 'IntrinsicAttributes' error during Turbopack builds.
 */
const ReactQuill = dynamic(() => import("react-quill"), { 
  ssr: false,
  loading: () => <div style={{ height: "40px", background: "#f9f9f9", borderRadius: "8px" }} />
}) as any;
...
export default function LeadControlRoom() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`chat:${id}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "chat_messages", filter: `lead_id=eq.${id}` }, 
  ...
  const handleSend = async () => {
    if (!input.trim() || !isHuman) return;
    const { error } = await supabase.from("chat_messages").insert([
      { lead_id: id, role: "admin", message: input.trim() }
    ]);
    if (!error) setInput("");
  };
    ...
    <div style={{ minHeight: "100vh", backgroundColor: brand.canvas, padding: "60px 80px", fontFamily: "'Nunito Sans', sans-serif" }}>
      ...
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "48px" }}>
        <div>
          ...
          <div style={{ display: "flex", gap: "16px", fontSize: "11px", fontWeight: 800, letterSpacing: "1.5px", color: brand.accentGold }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "32px" }}>
          ...
          <section style={cardStyle(brand.border)}>
            ...
            <div style={inputArea}>
              ...
              />
              <button onClick={handleSend} disabled={!isHuman} style={isHuman ? btnIcon(brand.corporateGreen) : btnDisabled}>
                <Send size={16} />
              </button>
            </div>
          </section>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "32px" }}>
          ...
          <section style={cardStyle(brand.border)}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "11px", fontWeight: 800, color: brand.primaryText }}>
              {/* Fix: Icon does not use 'title' prop */}
...
const chatBox = { height: "400px", overflowY: "auto" as const, display: "flex", flexDirection: "column" as const, gap: "12px", paddingRight: "10px" };
