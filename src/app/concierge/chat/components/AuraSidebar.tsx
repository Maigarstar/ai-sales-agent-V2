    ...
    <>
      ...
      >
        ...
        <div className="flex h-24 items-center justify-between px-8 shrink-0">
          ...
          <button
            onClick={onToggle}
            className={`p-2 rounded-full hover:bg-white/5 transition ${textColor}`}
          >
            <ChevronLeft size={20} />
          </button>
        </div>
          ...
          {userType === "business" ? (
            <div className="space-y-8">
              ...
              <nav className="space-y-2 pt-4 border-t border-white/10">
                <button className="w-full flex items-center gap-3 py-2 text-[11px] uppercase tracking-widest opacity-60 hover:opacity-100">
            ...
            <div className="space-y-10">
              ...
              <div>
                ...
                <div className="space-y-4">
                  {shortlist.length === 0 ? (
                    <p className="text-[10px] uppercase tracking-widest opacity-20 text-center py-4 italic">
                      Collection Empty
                    </p>
                  ) : (
                    shortlist.map((item) => (
                      <div
                        key={item.id}
                        className="group p-3 rounded-2xl border border-white/5 bg-white/5 transition relative"
                      >
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-xl overflow-hidden bg-black/20">
                            {item.image ? (
                              ...
                              <div className="w-full h-full flex items-center justify-center bg-[#C5A059]/10 text-[#C5A059] text-[8px] font-bold">
                                5*
                              </div>
                            )}
                          </div>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-1 opacity-40 text-[9px] uppercase mt-1">
                              <MapPin size={8} />
                          ...
                          <button
                            ...
                            className="opacity-0 group-hover:opacity-100 p-2 text-red-500/40 hover:text-red-500"
                          >
                            <Trash2 size={14} />
                          </button>
                        </div>
              ...
              <div>
                ...
                <ul className="space-y-3">
                  {["Summer Itinerary", "Private Travel Plans"].map((item) => (
                    <li
                      key={item}
                      className="text-[12px] opacity-60 hover:opacity-100 cursor-pointer flex items-center gap-3"
                    >
        ...
        <div className={`p-8 border-t ${isLightMode ? "border-black/5" : "border-white/5"}`}>
          {shortlist.length > 0 && userType === "client" && (
            <button
              onClick={onRequestProposal}
              className="w-full mb-6 py-4 rounded-xl bg-[#C5A059] text-black text-[10px] font-black uppercase tracking-[0.2em] shadow-lg hover:brightness-110"
            >
              Inquire Collection ({shortlist.length})
            </button>
          )}

          <button className="w-full flex items-center gap-3 py-2 text-[11px] uppercase tracking-widest opacity-40 hover:opacity-100">
