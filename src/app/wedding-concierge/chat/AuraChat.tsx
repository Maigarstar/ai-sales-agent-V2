...
export default function AuraChat({
  ...
  const fetchListings = async (q: string) => {
    ...
    try {
      ...
      setListings(Array.isArray(data.listings) ? data.listings.filter(Boolean) : []);
    } catch (err) {
  ...
  return (
    <div className="relative h-full w-full px-6 pb-12 overflow-y-auto">
      ...
      {/* Status */}
      {loading && (
        <p className="text-center text-sm opacity-60">Curating results…</p>
      )}
      ...
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-8 max-w-7xl mx-auto">
        {listings.map((listing, i) => (
          <AuraListingCard
            ...
            isSelected={shortlist.some((s) => s?.id === listing.id)}
